...
int main() {
    ...
    // calc image height using ratio, esnure its at least one
    int image_height = int(image_width / aspect_ratio);
    image_height = (image_height < 1) ? 1 : aspect_ratio;
    ...
    //Calc the vectors across hori and vert viewport edges
    ...
    for (int j = 0; j < image_height; j++)
    {
        ...
        {
            auto pixel_centre = pixel100_loc + (i * pixel_delta_u)
                                + (j * pixel_delta_v);
